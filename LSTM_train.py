...
if LSTM_STATEFUL:
    ...
    class LossHistory(keras.callbacks.Callback):
        ...
        def on_epoch_end(self, epoch, logs={}):
            ...
            loss_of_eposh = np.sqrt(self.batch_losses.sum()/batch_num)
    ...
    for epoch in range(EPOCH_SIZE):
        ...
        if ((epoch % SAVE_GAP) == 0):
            ...
            for sample_idx in range(VALID_SIZE):
                y_valid_out[sample_idx] = moe = model_valid.predict(
                    x_valid[sample_idx].reshape(1,1,input_dim), batch_size=1)
                model_valid.fit(x_valid[sample_idx].reshape(
                    1, 1, input_dim), y_valid[sample_idx], epochs=1, batch_size=1, verbose=0, shuffle=False)
    ...
    for e_net in existed_nets:
        if (os.path.isfile(e_net) and e_net.endswith('_net_model.h5')):
            counter += 1
    os.chdir("..")
    save_name = '{0:d}_{1:.4f}_{2:d}_stateful_net_model.h5'.format(
        counter, INIT_LEARNING_RATE, epoch + 1)
    if(early_stop_flag):
        os.rename('{0:s}{1:s}'.format(net_dir, temp_save_name),
                '{0:s}{1:s}'.format(net_dir, save_name))
    else:
        # torch.save(net.cpu().state_dict(),
        #         '{0:s}{1:s}'.format(net_dir, save_name))
        model.fit(x_valid,y_valid,epochs=1,batch_size=1,verbose=1)
        model.save('{0:s}{1:s}'.format(net_dir, save_name))
...
# else:
    # train_num = sample_num - VALID_SIZE - TEST_SIZE
    # assert (train_num % (BATCH_SIZE * TIME_STEPS) == 0) and (VALID_SIZE %
    #                                                           TIME_STEPS == 0) and (VALID_SIZE % TIME_STEPS == 0)
    # batch_num = train_num / (BATCH_SIZE * TIME_STEPS)

    # model = Sequential()
    # model.add(LSTM(units=LSTM_UNITS,input_shape=(TIME_STEPS,1),activation='tanh',return_sequences=True,dropout=LSTM_INPUT_DROPOUT,recurrent_dropout=LSTM_RECURRENT_DROPOUT))
    # model.add()
